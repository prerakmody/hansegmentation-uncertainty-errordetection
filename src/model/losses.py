...
def get_avu_threshold(array_ac, array_au, array_ic, array_iu, y_pred_unc, thresh_uncer, RATIO_N_AC):
    ...
    res = tf.math.log(1 + ((n_au + n_ic) / (n_ac*RATIO_N_AC + n_iu + _EPSILON)) ) # [B] -> [B] # values should be in the range of [ln(1), ln(1+[0.1,0.5])] -> [0, [0.09, 0.4]]
...
def loss_avu_3d_tf_func(y_true, y_pred, y_pred_unc=None, label_mask=[], weights=[]
                ...
                # , thresh_uncer=[0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65,0.7, 0.75, 0.8, 0.85, 0.9]
    ...
    RATIO_N_AC     = 0.1 # [1 if MC=1 & entropy, 0.01 if MC=5 & MI]
