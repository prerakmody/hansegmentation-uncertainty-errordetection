...
print ('\n\n ======================================================================== ')
print (' - Code for \"Improving Error Detection in Deep Learning based Radiotherapy Autocontouring using Bayesian Uncertainty\" ')
print (' --- Published in MICCAI-UNSURE Workshop, 2022')
print (' --- Authors: Prerak Mody (P.P.mody@lumc.nl), Marius Starting (M.Staring@lumc.nl)')
print (' --- Lab    : Division of Image Processing (LKEB), Leiden University Medical Centre, The Netherlands')
print (' ======================================================================== \n\n')

from pathlib import Path
PROJECT_DIR = Path(__file__).parent.absolute().parent.absolute()
MAIN_DIR    = Path(PROJECT_DIR).parent.absolute()
MODEL_DIR   = Path(PROJECT_DIR).joinpath('_models')
...
try:
    if len(tf.config.list_physical_devices('GPU')):
        ...
        print (' - [TFlow Build Info] ver: ', tf.__version__, 'CUDA(major.minor): ',  sys_details["cuda_version"], ' || cuDNN(major): ', sys_details["cudnn_version"], ' (Time taken: ', round(time.time() - t0,2),'s)')
        print ('\n')
    else:
        print ('\n - No GPU present!! Exiting ...')
...
KEY_KL_ALPLHA_INIT  = 'kl_alpha_init'
KEY_KL_SCHEDULE     = 'kl_schedule'
KEY_KL_SCALE_FACTOR = 'kl_scale_factor'

KEY_OPTIMIZER = 'optimizer'
KEY_INIT_LR   = 'init_lr'
KEY_FIXED_LR  = 'fixed_lr'
KEY_EPOCHS    = 'epochs'
KEY_EPOCHS_SAVE = 'epochs_save'
KEY_EPOCHS_EVAL = 'epochs_eval'
KEY_EPOCHS_VIZ  = 'epochs_viz'

KEY_LOAD_MODEL  = 'load_model'
KEY_LOAD_MODEL_BOOL = 'load_model_bool'
KEY_LOAD_EXP_NAME   = 'load_exp_name'
KEY_LOAD_EPOCH      = 'load_epoch'
KEY_LOAD_OPTIMIZER_LR = 'load_optimizer_lr' 

KEY_MODEL_PROFILER = 'profiler'
KEY_PROFILE_BOOL = 'profile_bool'
KEY_PROFILE_EPOCHS = 'profile_epochs'
KEY_PROFILE_STEPS_PER_EPOCH = 'profile_steps_per_epoch'
KEY_PROFILE_STARTING_STEP = 'starting_step'
KEY_MODEL_TBOARD = 'model_tboard'

KEY_DEEPSUP = False
KEY_UNCERTAINTY_METRIC = 'unc_metric'
...
MODEL_ONET_FLIPOUT_DENSEASPP = 'ModelONetFlipOutDenseASPP'
...
#                      METRIC RELATED                      #
############################################################
KEY_METRIC_TBOARD = 'logging_tboard'
KEY_METRICS_EVAL  = 'metrics_eval'
KEY_METRICS_LOSS  = 'metrics_loss'
...
KEY_LOSS_WEIGHTED = 'loss_weighted'
KEY_LOSS_MASK     = 'loss_mask'
KEY_LOSS_COMBO    = 'loss_combo'
KEY_LOSS_EPOCH    = 'loss_epoch'
KEY_LOSS_RATE     = 'loss_rate'
...
KEY_DIR_TYPE    = 'dir_type'
KEY_RESAMPLED   = 'resmapled'
KEY_CROP_INIT    = 'crop_init'
KEY_GRID         = 'grid'
KEY_RANDOM_GRID  = 'random_grid'
KEY_FILTER_GRID  = 'filter_grid'
KEY_CENTRED_PROB = 'centred_prob'
KEY_BATCH_SIZE   = 'batch_size'
KEY_SHUFFLE      = 'shuffle'
KEY_PREFETCH_BATCH = 'prefetch_batch'
KEY_PARALLEL_CALLS = 'parallel_calls'
